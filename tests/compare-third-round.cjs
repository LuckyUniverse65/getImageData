...
const browser=read('out/cdp-third-round-browser.json');
assert.equal(browser.browserMode,'user-chrome-cdp');
assert.equal(browser.script,'tests/third-round-cases.js');
assert.equal(browser.scriptSHA256,hash(browser.script));
const local=require('./third-round-cases')(require('..').OffscreenCanvas);
