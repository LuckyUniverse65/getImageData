...
if(process.argv.includes('--local-child')){
    Promise.resolve().then(()=>require('./fourth-round-cases').runNinthRoundCases(require('..').OffscreenCanvas))
        .then(value=>console.log(JSON.stringify(value))).catch(e=>{console.error(e);process.exitCode=1;});
}else{
    const browser=JSON.parse(fs.readFileSync(path.join(root,'out/cdp-ninth-round-browser.json'),'utf8'));
    ...
    const local=JSON.parse(child.stdout.trim()),reference=browser.value.ninthRound;
    ...
    fs.writeFileSync(path.join(root,'out/cdp-ninth-round-local.json'),JSON.stringify(local,null,2)+'\n');
    fs.writeFileSync(path.join(root,'out/cdp-ninth-round-diff.json'),JSON.stringify(report,null,2)+'\n');
