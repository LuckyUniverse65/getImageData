...
if(process.argv.includes('--local-child')){
    // Reentrant JS can reach native code; isolate possible native failures.
    console.log(JSON.stringify(require('./fourth-round-cases').runFifthRoundCases(require('..').OffscreenCanvas)));
}else{
    const browser=JSON.parse(fs.readFileSync(path.join(root,'out/cdp-fifth-round-browser.json'),'utf8'));
    ...
    const local=JSON.parse(child.stdout.trim()),reference=browser.value.fifthRound;
    ...
    fs.writeFileSync(path.join(root,'out/cdp-fifth-round-local.json'),JSON.stringify(local,null,2)+'\n');
    fs.writeFileSync(path.join(root,'out/cdp-fifth-round-diff.json'),JSON.stringify(report,null,2)+'\n');
