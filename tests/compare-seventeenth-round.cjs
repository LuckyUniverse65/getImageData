...
const local = require('./fourth-round-cases').runSeventeenthRoundCases(require('..').OffscreenCanvas);
if (!process.argv.includes('--local-child')) fs.writeFileSync(path.join(root, 'out/cdp-seventeenth-round-local.json'), JSON.stringify(local) + '\n');
if (process.argv.includes('--local-child')) {
    ...
    const browser = JSON.parse(fs.readFileSync(path.join(root, 'out/cdp-seventeenth-round-browser.json'), 'utf8'));
    ...
    const reference = browser.value.seventeenthRound;
    ...
    fs.writeFileSync(path.join(root, 'out/cdp-seventeenth-round-diff.json'), JSON.stringify(report, null, 2) + '\n');
