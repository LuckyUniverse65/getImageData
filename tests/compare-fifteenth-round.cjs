...
if(process.argv.includes('--local-child')){
    Promise.resolve().then(()=>require('./fourth-round-cases').runFifteenthRoundCases(require('..').OffscreenCanvas))
        .then(value=>console.log(JSON.stringify(value))).catch(e=>{console.error(e);process.exitCode=1;});
}else{
    const browser=JSON.parse(fs.readFileSync(path.join(root,'out/cdp-fifteenth-round-browser.json'),'utf8'));
    ...
    const local=JSON.parse(child.stdout.trim()),reference=browser.value.fifteenthRound;
    ...
    for(const name of Object.keys(local)){
        if(isDeepStrictEqual(local[name],reference[name]))matches.push(name);
    ...
    fs.writeFileSync(path.join(root,'out/cdp-fifteenth-round-local.json'),JSON.stringify(local,null,2)+'\n');
    fs.writeFileSync(path.join(root,'out/cdp-fifteenth-round-diff.json'),JSON.stringify(report,null,2)+'\n');
