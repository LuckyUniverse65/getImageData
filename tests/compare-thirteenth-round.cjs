...
if(process.argv.includes('--local-child')){
    Promise.resolve().then(()=>require('./fourth-round-cases').runThirteenthRoundCases(require('..').OffscreenCanvas))
        .then(value=>console.log(JSON.stringify(value))).catch(e=>{console.error(e);process.exitCode=1;});
}else{
    const browser=JSON.parse(fs.readFileSync(path.join(root,'out/cdp-thirteenth-round-browser.json'),'utf8'));
    ...
    const local=JSON.parse(child.stdout.trim()),reference=browser.value.thirteenthRound;
    ...
    fs.writeFileSync(path.join(root,'out/cdp-thirteenth-round-local.json'),JSON.stringify(local,null,2)+'\n');
    fs.writeFileSync(path.join(root,'out/cdp-thirteenth-round-diff.json'),JSON.stringify(report,null,2)+'\n');
