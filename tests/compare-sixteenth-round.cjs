...
const local = require('./fourth-round-cases').runSixteenthRoundCases(require('..').OffscreenCanvas);
if (!process.argv.includes('--local-child')) fs.writeFileSync(path.join(root, 'out/cdp-sixteenth-round-local.json'), JSON.stringify(local) + '\n');
if (process.argv.includes('--local-child')) {
    ...
    const browser = JSON.parse(fs.readFileSync(path.join(root, 'out/cdp-sixteenth-round-browser.json'), 'utf8'));
    ...
    const reference = browser.value.sixteenthRound;
    ...
    fs.writeFileSync(path.join(root, 'out/cdp-sixteenth-round-diff.json'), JSON.stringify(report, null, 2) + '\n');
