...
unsafe fn canvas_2d_from(env: NapiEnv, object: NapiValue) -> Option<&'static mut Canvas2D> {
    ...
    if (api().check_object_type_tag)(env,object,&CANVAS_2D_TYPE_TAG,&mut tagged)!=0 || !tagged { return None; }
    let mut raw = ptr::null_mut();
    if (api().unwrap)(env, object, &mut raw) != 0 || raw.is_null() {
        return None;
    }
...
unsafe fn canvas_2d(width: usize, height: usize, env: NapiEnv, alpha: bool, desynchronized: bool, will_read_frequently: bool) -> NapiValue {
    ...
    (api().type_tag_object)(env,object,&CANVAS_2D_TYPE_TAG);
    ...
    let props = [
        ...
        ("direction", 17), ("letterSpacing",18), ("wordSpacing",19), ("fontKerning",20), ("imageSmoothingQuality",21), ("fontStretch",22), ("fontVariantCaps",23), ("textRendering",24),
    ];
...
unsafe extern "C" fn set_canvas_property(env: NapiEnv, info: NapiCallbackInfo) -> NapiValue {
    ...
    let prepared=if [0,1,2,6,7,8,9,12,13,17,18,19,20,21,22,23,24].contains(&data){
    ...
    match name {
        ...
        "arcTo" => {
            if args.len()>=5 {
                ...
                if radius < 0.0 { return dom_error(env,"IndexSizeError","Negative arcTo radius"); }
        ...
        "clip" => { let evenodd=args.first().and_then(|value|value_string(env,*value)).as_deref()==Some("evenodd"); let paths=canvas.path.clone(); canvas.clip.push(paths); if !canvas.native.is_null(){skia_canvas_clip(canvas.native,evenodd as i32);} undefined(env) }
        _ => undefined(env),
    }
}
...
unsafe extern "C" fn canvas_method(env: NapiEnv, info: NapiCallbackInfo) -> NapiValue {
    ...
    match args.first().and_then(|value| value_string(env, *value)).as_deref() {
        Some("2d") => {
            ...
            canvas_2d(width as usize, height as usize, env, context_option(env, options, "alpha", true),
...
unsafe extern "C" fn canvas_2d_constructor(env: NapiEnv, _: NapiCallbackInfo) -> NapiValue {
    canvas_2d(300, 150, env, true, false, false)
}
